...
def test_appetite_max_stitch_unit() -> None:
    """Appetite MAX: hybrid+CSE stitch helper."""
    assert UNIT_APPETITE_MAX_MTS.is_file(), f"missing {UNIT_APPETITE_MAX_MTS}"
    _require_web_node_modules()
    npx = _npx()
    staged = WEB / ".web_appetite_max_unit.mts"
    staged.write_text(UNIT_APPETITE_MAX_MTS.read_text(encoding="utf-8"), encoding="utf-8")
    try:
        proc = subprocess.run(
            [npx, "--yes", "tsx", str(staged.name)],
            cwd=str(WEB),
    ...
    if proc.returncode != 0:
        pytest.fail(
            f".web_appetite_max_unit.mts failed ({proc.returncode}):\n"
            f"stdout:\n{proc.stdout}\nstderr:\n{proc.stderr}"
        )
