"""Wave36: medium+ bugs — SSR loopback, HEALTH_URL SSRF, apiMutate, JSON body.
...
4. Mutating routes (demo / alerts / watchlist) must stream-bound JSON via
   ...
   / ``arrayBuffer()`` used to OOM Node on huge POSTs.
...
7. Alerts ``?active=`` must accept only exact ``true``/``false``/omit.
"""
...
def test_mutating_routes_bound_json_body() -> None:
    ...
    for path in routes:
        ...
        assert "Request body too large" in source, path.name
...
def test_alerts_active_param_strict() -> None:
    source = (WEB / "src" / "app" / "api" / "v1" / "alerts" / "route.ts").read_text(
        encoding="utf-8"
    )
    assert 'activeParam === "true"' in source
    assert 'activeParam === "false"' in source
    assert 'activeParam === null || activeParam === "true"' not in source
    assert "active must be true or false" in source
