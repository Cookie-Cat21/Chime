...
Wave 91 lands focused bool numeric / CLI / disclosure / health / alert-lookup harden and refreshes the loop snapshot toward w92–w100.
...
## Wave 91 — Bool numeric / CLI / disclosure / health / alert lookup harden

**Theme:** Document the real Wave 91 sibling fixes that landed after the wait/pull: bool numeric rule coercion, CLI/migrate arg soft-accepts, disclosure watermark conflict, health poller merge, alert lookup NFA, and CSE numeric payload bool rejection. No fake SHAs; uncommitted parallel work is not counted here.
...
| _(this)_ | docs(w91): include late sibling commits |
...
- `LOOP_STATUS.md` — waves completed through w91; pre-correction HEAD `89d6630d`; commits ahead of `main` `306+`; next steps point at w92–w100.
...
| 91 (`w91`) | 6+ (bool numeric + CLI/disclosure/health/alert/CSE harden + report) |
| **Total** | **100+** |
...
- Wave 92+ only on **new** medium+ fuel — PG claim/lock/health/count soft-accept hunting exhausted (w83); CDN soft-accept closed (late w83); CSE HTTP classify soft-accepts closed (w89/w90); clock-skew claim CLEAN (w87); Wave 91 bool/CLI/disclosure/health/alert/CSE fixes landed. Do not farm duplicate pins.
