...
| ID | Item | Status |
|---|---|---|
| E10-Q01 | CSRF header≠cookie mismatch → 400 csrf_failed unit | OPEN |
| E10-Q02 | Logout happy-path clears session+CSRF cookies (RUN_WEB) | OPEN |
| E10-Q03 | Contract order: missing session → 401 before CSRF check (doc+test) | OPEN |
| E10-D01 | Empty disclosures state on symbol detail | OPEN |
| E10-D02 | Alerts history empty copy when no fires | OPEN |
| E10-O01 | Makefile/factory-verify smoke for portfolio_sum.py | OPEN |
| E10-A01 | API_CONTRACT note: 401 beats csrf_failed when both would apply | OPEN |
