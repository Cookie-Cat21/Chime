# Epoch 13 Board — Thin polish + ops DX (pre-seed)

**Status:** OPEN

Fence-legal only. Theme: remaining ACCEPT-DEFER thin slices, dash copy,
ops DX. No portfolio P&L / screener / TA / payments / native app / competitor
scrape / cse.lk from `web/`. Activate via `make factory-refill` after Epoch 12.

| ID | Item | Status |
|---|---|---|
| E13-B01 | /myalerts empty-state copy mentions /alert examples + NFA | DONE |
| E13-B02 | /mywatchlist empty-state copy + /watch hint | DONE |
| E13-D01 | Login page NFA + short product explainer (no dashboard clutter) | DONE |
| E13-D02 | Alerts page empty active-rules copy + create affordance | DONE |
| E13-Q01 | Unit: disclosure rule missing created_at fails closed | DONE |
| E13-O01 | HANDOFF note Epoch 12→13 refill + verify unit path | DONE |
| E13-C01 | tradeSummary miss ops signal (log + health detail when empty-OK contrast) | DONE |
| E13-A01 | SESSION_LOG entry for Epoch 11–12 drain | DONE |

Never farm. One concern per commit.
