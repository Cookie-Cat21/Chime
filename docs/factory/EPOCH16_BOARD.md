# Epoch 16 Board — Thin polish (pre-seed)

**Status:** STAGED

Fence-legal only. Theme: thin polish + docs/ops DX. No portfolio P&L /
screener / TA / payments / native app / competitor scrape / cse.lk from `web/`.
Activate via `make factory-refill` after Epoch 15.

| ID | Item | Status |
|---|---|---|
| E16-Q01 | Unit: dual claim_unsent SKIP LOCKED no duplicate rows | OPEN |
| E16-Q02 | Unit: off-hours unsent drain still attempts delivery | OPEN |
| E16-D01 | Nav active-state a11y for health/watchlist | OPEN |
| E16-D02 | Login error copy stays brand-tone + NFA | OPEN |
| E16-O01 | HANDOFF Epoch 15→16 refill path | OPEN |
| E16-C01 | CSE client timeout log includes symbol when known | OPEN |
| E16-A01 | SESSION_LOG Epoch 15 drain entry | OPEN |
| E16-B01 | /unwatch not-on-list reply actionable | OPEN |

Never farm. One concern per commit.
